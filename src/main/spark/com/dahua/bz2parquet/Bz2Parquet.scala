...
object Bz2Parquet {
  def main(args: Array[String]): Unit = {
    ...
    //对象  sparkContetxt sparksession
    val conf = new SparkConf().set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
    val spark: SparkSession = SparkSession.builder().appName("Bz2Parquet").master("local[*]").getOrCreate()
    val sc: SparkContext = spark.sparkContext
    import  spark.implicits._

    val value: RDD[String] = sc.textFile(loginputpath)

    val value1: RDD[Array[String]] = value.map(_.split(",", -1))

    val value2: RDD[Array[String]] = value1.filter(_.length >= 85)
    println(value2.collect().toBuffer)
    val row: RDD[Row] = value2.map(line => {
      Row(line(0),
    ...
    val frame: DataFrame = spark.createDataFrame(row.repartition(1), SchemaUtil.logStructType)
   frame.write.parquet(logoutputpath)
